...
import { JSON_MOVES, JSON_SHEET, TEX_CLAWD, TEX_CLAWD_ALT } from './PreloadScene';
...
export class FightScene extends Phaser.Scene {
  ...
  private fight: FightData = { mode: 'cpu', demo: false };
  private startedAt = 0;
  private endedAt = 0;
  ...
  init(data: Partial<FightData>): void {
    this.fight = { mode: data.mode ?? 'cpu', demo: data.demo ?? false };
    this.accumulator = 0;
    this.paused = false;
    this.previousPhase = '';
    this.endedAt = 0;
  ...
  create(): void {
    ...
    const animMap = new AnimMap(this.cache.json.get(JSON_SHEET), this.cache.json.get(JSON_MOVES));

    this.match = new Match(this.settings.names[0], this.settings.names[1]);
    ...
    this.views = [
      new FighterView(this, this.match.fighters[0], animMap, TEX_CLAWD),
      new FighterView(this, this.match.fighters[1], animMap, TEX_CLAWD_ALT),
    ];

    this.fx = new Fx(this);
    this.hud = new Hud(this, this.match);
    this.hud.setNames(this.settings.names[0], this.settings.names[1]);
  ...
  override update(time: number, delta: number): void {
    ...
    this.views[0].update(frameDelta);
    this.views[1].update(frameDelta);
