# Muse — first sprite-sheet pass

42 frames across seven sequences, based on the approved cream plush Muse
concept with its blue chest mark.

## Files

- `muse-sheet.png`: transparent 1344 × 1120 atlas, six columns and seven rows.
- `muse-sheet.json`: frame rectangles, durations and named animation tags.
- `muse-moves.json`: art timing, loops, return-to-idle and final-pose hold
...
- `muse.ase` / `muse.aseprite`: editable timeline with one artwork layer.
- `strips/`: a six-frame transparent PNG strip for each animation.
- `frames/`: individual PNG frames. `muse-master.png` is the neutral guard.

Each frame is 224 × 160, right-facing, with origin (106,136). Keep the full
canvas and use nearest-neighbor scaling. The palette has 18 opaque colours
...
Muse is playable in the browser build. `scripts/stage-fighter.py muse`
writes the per-cel metadata above and copies the atlas and both JSON files
into `web/public/assets/muse/`, which is what the browser loads. The `.ase`
