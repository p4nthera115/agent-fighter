...
assert count == len(metadata['frames']) == 56
offset = 128
...
for frame_index in range(count):
    ...
    for _ in range(chunks):
        ...
        offset += size
    assert offset == end
    assert len(cels) == len(layer_names) == 9
...
assert tags == [('idle',0,5),('punch',6,14),('uppercut',15,23),('kick',24,32),('damage',33,38),('victory',39,46),('defeated',47,55)]
assert all(decoded[i]==decoded[0] for i in [5,6,14,15,23,24,32]), 'Loops must return to master'
assert decoded[38] == decoded[0], 'Damage must recover to idle'
assert decoded[39] == decoded[46], 'Victory loop seam'
assert decoded[54] == decoded[55], 'Defeated must hold still'
assert len(colors) <= 13
assert {c[3] for c in colors} == {0,255}, 'No semitransparent pixels'
report = dict(frames=count,layers=layer_names,tags=tags,canvas=[w,h],colors_including_transparency=len(colors),pixel_comparison='All 56 editable-file composites exactly match individual PNGs and atlas',baseline=136,loop_seams='Attack loops and damage recovery return to master; victory loops; defeated holds final pose',application_check='Neither LibreSprite nor Aseprite installed; application-level open not tested')
