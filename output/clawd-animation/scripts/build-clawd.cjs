...
async function main(){
  ...
  const poses=[
    ...
    {tag:'damage',phase:'impact',ms:50,bob:1,lean:-5,hand:-3,handY:5,eyes:'shock'},
    {tag:'damage',phase:'recoil',ms:67,bob:3,lean:-12,hand:-9,handY:10,guard:6,eyes:'hurt'},
    {tag:'damage',phase:'brace',ms:83,bob:5,lean:-9,hand:-6,handY:9,guard:5,eyes:'hurt'},
    {tag:'damage',phase:'shake off',ms:83,bob:2,lean:-5,hand:-4,handY:4,eyes:'closed'},
    {tag:'damage',phase:'recover',ms:67,bob:1,lean:-2,eyes:'closed'},
    {tag:'damage',phase:'ready',ms:100,bob:0},
    {tag:'victory',phase:'fists up',ms:100,bob:0,celebrate:0,eyes:'happy'},
    {tag:'victory',phase:'cheer',ms:100,bob:-3,celebrate:4,eyes:'happy'},
    {tag:'victory',phase:'big cheer',ms:130,bob:-5,celebrate:8,eyes:'happy'},
    {tag:'victory',phase:'hold',ms:150,bob:-5,celebrate:8,eyes:'happy'},
    {tag:'victory',phase:'bounce down',ms:100,bob:-2,celebrate:3,eyes:'closed'},
    {tag:'victory',phase:'settle',ms:100,bob:0,celebrate:0,eyes:'happy'},
    {tag:'victory',phase:'second pump',ms:100,bob:-2,celebrate:3,eyes:'happy'},
    {tag:'victory',phase:'cheer loop',ms:140,bob:0,celebrate:0,eyes:'happy'},
    {tag:'defeated',phase:'last hit',ms:67,bob:2,lean:-7,eyes:'shock',handY:9},
    {tag:'defeated',phase:'stagger',ms:100,bob:5,lean:-10,eyes:'hurt',handY:15},
    {tag:'defeated',phase:'eyes dazed',ms:150,bob:7,lean:-6,eyes:'dazed',handY:18},
    {tag:'defeated',phase:'knees buckle',ms:100,collapse:.25,eyes:'dazed'},
    {tag:'defeated',phase:'fall',ms:67,collapse:.65,eyes:'dazed'},
    {tag:'defeated',phase:'floor impact',ms:83,collapse:1,eyes:'dazed'},
    {tag:'defeated',phase:'small rebound',ms:100,collapse:.85,eyes:'dazed'},
    {tag:'defeated',phase:'down',ms:150,collapse:1,eyes:'dazed'},
    {tag:'defeated',phase:'defeated hold',ms:500,collapse:1,eyes:'dazed'},
  ];
  ...
  // Separate the original eyes without changing any existing normal pose pixels.
  const cleanBody={...parts.body,data:Buffer.from(parts.body.data)};
  const normalEyes={...parts.body,data:Buffer.alloc(parts.body.data.length)};
  for(let y=18;y<56;y++)for(let x=15;x<112;x++){
    const k=(y*parts.body.w+x)*4;
    if(((x>=34&&x<=49&&y>=34&&y<=52)||(x>=92&&x<=108&&y>=27&&y<=45))&&parts.body.data[k+3]){
      parts.body.data.copy(normalEyes.data,k,k,k+4);
      const c=palette[6];cleanBody.data.set([...c,255],k);
    }
  }
  function expression(kind){
    if(!kind)return normalEyes;
    const part={...normalEyes,data:Buffer.alloc(normalEyes.data.length)};
    const dot=(x,y)=>{if(x>=0&&y>=0&&x<part.w&&y<part.h)part.data.set([...palette[0],255],(y*part.w+x)*4);};
    const line=(x0,y0,x1,y1)=>{const n=Math.max(Math.abs(x1-x0),Math.abs(y1-y0));for(let i=0;i<=n;i++){const x=Math.round(x0+(x1-x0)*i/n),y=Math.round(y0+(y1-y0)*i/n);for(let a=0;a<3;a++)for(let b=0;b<3;b++)dot(x+a,y+b);}};
    for(const [x,y] of [[33,34],[91,27]]){
      if(kind==='happy'){line(x,y+7,x+5,y);line(x+5,y,x+10,y+5);}
      if(kind==='closed')line(x,y+5,x+11,y+3);
      if(kind==='dazed'){line(x,y,x+10,y+9);line(x+10,y,x,y+9);}
      if(kind==='hurt'){const left=x<60;line(x+(left?0:10),y,x+(left?8:2),y+5);line(x+(left?8:2),y+5,x+(left?0:10),y+10);}
      if(kind==='shock'){line(x,y,x+9,y-1);line(x+9,y-1,x+9,y+11);line(x+9,y+11,x,y+12);line(x,y+12,x,y);}
    }
    return part;
  }
  // A crescent follows the fist's path. It lasts only through the fast rise,
  // with a thinner broken tail on the next cel; it is not a stretched limb.
  function arc(target,stage){
    const polygons=stage===0 ? [
      [[153,96],[171,86],[184,72],[187,59],[184,48],[178,42],[182,57],[177,72],[166,86]],
    ] : stage===1 ? [
      [[150,97],[172,85],[192,63],[201,42],[198,23],[187,11],[188,29],[184,48],[175,68],[162,86]],
      [[160,87],[180,66],[190,43],[192,27],[190,20],[196,30],[195,46],[184,69]],
    ] : [
      [[172,70],[184,52],[191,31],[187,17],[192,23],[195,36],[186,55]],
      [[157,90],[165,81],[171,73],[168,82]],
    ];
    polygons.forEach((points,index)=>{
      for(let y=0;y<H;y++)for(let x=0;x<W;x++){
        ...
        if(inside)target.set([...palette[index?11:9],255],(y*W+x)*4);
      }
    });
  }
  const layerNames=['Foot 1 - outward','Foot 2','Foot 3','Foot 4 - kick','Attack trail','Body','Left hand','Right hand','Eyes'];
  const frames=[], layers=[];
  ...
  for(let i=0;i<poses.length;i++){
    ...
    if(p.collapse){
      const c=p.collapse, bw=Math.round(126+16*c),bh=Math.round(89-49*c),bx=Math.round(43-8*c),by=126-bh;
      for(let j=0;j<4;j++)stamp(ls[j],rotateCCW(parts['foot-'+(j+1)]),Math.round(37+j*35),Math.round(102+16*c),28,Math.round(34-16*c));
      stamp(ls[5],cleanBody,bx,by,bw,bh);
      stamp(ls[8],expression(p.eyes),bx,by,bw,bh);
      stamp(ls[6],parts['left-hand'],12,Math.round(83+29*c),38,Math.round(41-17*c));
      stamp(ls[7],parts['right-hand'],172,Math.round(91+21*c),36,Math.round(36-12*c));
    }else{
    for(let j=0;j<4;j++){
      if(j===3&&p.kick)stamp(ls[j],kickFoot,p.kick.x,p.kick.y,p.kick.w,p.kick.h);
      else {const rise=['uppercut','victory'].includes(p.tag)?Math.min(0,bob):0;stamp(ls[j],parts['foot-'+(j+1)],44+j*31,102+rise,24,34-rise);}
    }
    if(p.extension)stamp(ls[4],parts['right-hand'],151+Math.max(lean,0),61+bob+(p.handY||0),p.extension,23);
    if(p.upper&&i<=20)arc(ls[4],i-18);
    stamp(ls[5],cleanBody,43+lean,18+bob);
    stamp(ls[8],expression(p.eyes),43+lean,18+bob);
    stamp(ls[6],parts['left-hand'],17+Math.round(lean*.4),p.celebrate!==undefined?31-p.celebrate:71+bob+(p.guard||0));
    if(p.upper)stamp(ls[7],upFist,p.upper.x,p.upper.y);
    else stamp(ls[7],parts['right-hand'],157+(p.hand||0),p.celebrate!==undefined?18-p.celebrate:55+bob+(p.handY||0)+(p.guard||0));
    }
  ...
  meta.meta.frameTags=[...new Set(poses.map(p=>p.tag))].map(name=>({name,from:poses.findIndex(p=>p.tag===name),to:poses.findLastIndex(p=>p.tag===name),direction:'forward'}));
  ...
  const moves=JSON.parse(fs.readFileSync(path.join(out,'clawd-moves.json')));
  moves.animations=Object.fromEntries(meta.meta.frameTags.map(t=>[t.name,{loop:['idle','victory'].includes(t.name),from:t.from,to:t.to,...(!['idle','victory','defeated'].includes(t.name)?{next:'idle'}:{}),...(t.name==='defeated'?{holdLast:true}:{})}]));
  fs.writeFileSync(path.join(out,'clawd-moves.json'),JSON.stringify(moves,null,2));
  ...
  firstChunks.push(chunk(0x2018,Buffer.concat([tagsHead,...meta.meta.frameTags.map(t=>{const b=Buffer.alloc(17);b.writeUInt16LE(t.from);b.writeUInt16LE(t.to,2);b.writeUInt16LE(['idle','victory'].includes(t.name)?0:1,5);b[13]=238;b[14]=121;b[15]=80;return Buffer.concat([b,str(t.name)]);})])));
  ...
  console.log(`Created ${poses.length} frames, ${layerNames.length} editable layers, 12 opaque colors + transparency.`);
