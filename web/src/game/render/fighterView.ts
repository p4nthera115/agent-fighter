...
import type { FighterArt } from '../roster';
 ...
 *
 * The pivot and the cel size come from the fighter's own `-moves.json`, and
 * the shadow from its roster entry, so two different sheets can stand on the
 * same floor line without either one being the special case.
 */
export class FighterView {
  ...
  constructor(
    ...
    this.shadow = scene.add
      .ellipse(fighter.x, FLOOR_Y + 2, shadow.width, shadow.height, shadow.color, shadow.alpha)
      .setDepth(4);
  ...
  update(delta: number, celebrating = false): void {
    const f = this.fighter;
    this.celebrating = celebrating && !f.airborne && f.action !== 'ko';
    const visualState = this.celebrating ? 'victory' : f.action;
    // Restart a reaction on a new hit, including a second hit during hitstun.
    if (visualState !== this.visualState || (f.action === 'hitstun' && f.stunTimer > this.previousStun)) {
      this.visualState = visualState;
      this.stateElapsed = 0;
    } else if (f.hitstop === 0) this.stateElapsed += delta;
    this.previousStun = f.stunTimer;
    ...
    this.shadow.setDisplaySize(shadow.width * lift, shadow.height * lift);
    this.shadow.setAlpha(shadow.alpha * lift);
  }

  /**
   * States with no clip of their own borrow an idle pose, by position rather
   * than by index, so a six-cel idle and a nine-cel idle both work.
   */
  private pickFrame(): string {
    const f = this.fighter;
    const idle = this.anims.clip('idle');
    if (this.celebrating) return this.anims.frameAtTime('victory', this.stateElapsed);

    switch (f.action) {
      case 'attack':
        return this.anims.attackFrame(f);
      case 'knockdown':
        return this.anims.pose('damage', 2);
      case 'ko':
        return this.anims.frameAtTime('defeated', this.stateElapsed, false);
      case 'wakeup':
        return this.anims.pose('idle', 2);
      case 'hitstun':
        return this.anims.frameAtTime('damage', this.stateElapsed, false);
      case 'blockstun':
        return this.anims.pose('idle', -1);
      default:
        break;
    }

    if (f.airborne) return this.anims.pose('idle', f.vy > 0 ? 3 : 4);
    if (f.stance === 'crouch') return idle.names[0];
    if (f.blocking) return this.anims.pose('idle', -1);
  ...
  private pickDeformation(): { scaleX: number; scaleY: number; rotation: number } {
    ...
    let scaleY = 1;
    let rotation = 0;
    // These cels already contain their own squash and pose changes.
    if (this.celebrating || f.action === 'ko' || f.action === 'hitstun') return { scaleX, scaleY, rotation };

    if (f.action === 'knockdown') {
      const progress =
        1 - Phaser.Math.Clamp(f.stunTimer / 26, 0, 1);
